...
export const Recurring: React.FC = () => {
  ...
  useEffect(() => {
    if (editingId && formData.amount) {
      setBaseAmount(formData.amount);
    }
  }, [editingId, formData.amount]);

  const calculateTotal = (base: number, type: 'Inclusive' | 'Exclusive', rate: number) => {
    if (!base) return 0;
    let total = base;
    if (type === 'Exclusive') {
  ...
  const handleAmountChange = (val: number) => {
    setBaseAmount(val);
    const total = calculateTotal(val, formData.taxType || 'Exclusive', formData.taxRate || 0);
    setFormData(prev => ({ ...prev, amount: total }));
  };

  const handleTaxChange = (type: 'Inclusive' | 'Exclusive', rate: number) => {
    const total = calculateTotal(baseAmount, type, rate);
    setFormData(prev => ({ ...prev, taxType: type, taxRate: rate, amount: total }));
  };
              ...
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${item.type === 'Income' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                      }`}>
                      {item.type}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <button
                      ...
                      className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium transition-colors ${item.status === 'Active'
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                {formData.taxType === 'Inclusive' ? 'Total' : 'Base'} Amount ({currency})
              </label>
              <input
                type="number"
                required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={e => handleTaxChange(e.target.value as any, formData.taxRate || 0)}
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={e => handleTaxChange(formData.taxType || 'Exclusive', Number(e.target.value))}
              />
            </div>
          </div>
